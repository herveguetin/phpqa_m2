...
class Phpcs extends \Edge\QA\Tools\Tool
{
    ...
    protected function buildPhpcs(array $installedStandards)
    {
        ...
        $this->updateArgsForMagento($args);

        return $args;
    }

    private function updateArgsForMagento(&$args) {
        $args['severity'] = $this->config->value('phpcs.severity') ?: 1;
        $args['extensions'] = $this->config->value('phpcs.extensions') ?: $args['extensions'];
    }
}
